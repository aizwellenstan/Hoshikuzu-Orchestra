...

{
    ...
    {
        var slider = layer.Effects.addProperty("スライダー制御");
  ...
  // ここを変更
  function applyChannelToLayer(channel,layer,pb,totalNotesCount,notesSoFar,myComp)
  {
    ...
    for(var i = 0; i < channel.notes.length; i++)
    {
      ...
      if(vel && typeof(dur) != "undefined")
      {
        ...
        target("scale").setValue([vel*1.3,vel*1.3]);
        target.name = "note_"+pitch;
  ...
  function applyMidiToLayer(midi,layer,totalNotesCount,myComp)
  {
        ...
        for(var chKey in midi.channels)
        {
            ...
            {
                applyChannelToLayer(channel,layer,pb,totalNotesCount,notesSoFar,myComp);
    ...
    function apply(result)
    {
        ...
        if(midi.notes.length > 0)
        {
            ...
            applyMidiToLayer(midi,newLayer,totalNotesCount,myComp);
        }
...
// プロジェクトからletter_{s}を検索してindexを返す
function searchLetterIndex(letter){
  var num=0;
  var str="borders_"+letter+"_light";
  ...
  if(num == 0){
    // 存在しなかった場合、とりあえず「あ」を入れる
    var str="borders_あ_light";
    for (var i=1; i<=app.project.items.length ; i++){
      if (app.project.item(i).name == str){
        num = i;
        break;
...
// CSV選ばせて歌詞を読み込む
function getLilycs(){
  var tmpLilycs = new Array();
  csvFilePath = File.openDialog("CSVファイルを選択してください","");
  var myFile = new File(csvFilePath);
  if (myFile.open("r")) {
    while (! myFile.eof) {
      var curln = myFile.readln().split(",");
      var tmp = {};
      tmp.letter = curln[0];
      tmp.words = curln[1];
      tmpLilycs.push(tmp);
